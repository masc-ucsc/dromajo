/*
 * Code based from UCSC CSE220 class project (Muhammad Hadir Khan). Code has
 * several functions cut&pasted from https://github.com/bluespec/RISCV_gdbstub
 *
...
static int           gdb_hartid = 0;  // CPU/hartid to gdb
static RISCVMachine *gdb_m      = 0;
...
void val_to_hex16(const uint64_t val, const uint8_t xlen, char *buf) {
    assert((xlen == 8) || (xlen == 16) || (xlen == 32) || (xlen == 64));
...
uint8_t gdb_checksum(const char *buf, const size_t size) {
    uint8_t c = 0;
    size_t  j;
    for (j = 0; j < size; j++) c = (uint8_t)(c + ((uint8_t *)buf)[j]);
...
void handle_rsp_c() {
    ...
    while(1) { // should be while loops of steps, as this can stop due to breakpoints
        ...
        last_pc = virt_machine_get_pc(gdb_m, 0);
    }
...
void handle_rsp_g(const char *buf, const size_t buf_len) {
    ...
    for (int i = 0; i < 32; i++) {
        uint64_t v = virt_machine_get_reg(gdb_m, gdb_hartid, i);
        val_to_hex16(v, 64, &(response[i * 16]));
    }
...
void handle_rsp_G(const char *buf, const size_t buf_len) {
    ...
    // all riscv 32 registers - except x0
    for (int i = 1; i < 32; i++) {
        uint64_t val = 0;

        for (int j = 0; i < 8; j++) {
            char byte[3];
            const char *ptr = &buf[i * 8 + j * 2];
            strncpy(byte, ptr, 2);
            val += strtol(byte, NULL, 16) << i * 8;
        }

        riscv_set_reg(cpu, i, val);
    }
...
void handle_rsp_p(const char *buf, const size_t buf_len) {
    ...
    uint64_t v;
    if (reg == 0x20)                        // PC
        v = virt_machine_get_pc(gdb_m, gdb_hartid);
    else if (reg < 0x20)                    // xreg
        v = virt_machine_get_reg(gdb_m, gdb_hartid, reg + 1);
    else if (reg > 0x20 && reg <= 0x40)     // freg
        v = virt_machine_get_fpreg(gdb_m, gdb_hartid, reg - 0x21);
    else {
        RISCVCPUState *cpu = gdb_m->cpu_state[gdb_hartid];
        switch (reg) {
#if FLEN > 0
            ...
            case 0x43:
                ...
                break;
            case 0x44:
                v = cpu->frm << 5 | cpu->fflags;
                break;
#endif
            default:
                char resp[] = "E 2";  // out of bounds register number???
                send_rsp_pkt_to_gdb(resp, strlen(resp));
                return;
        }
    }
...
void handle_rsp_q(const char *buf, const size_t buf_len) {
    printf("The handle rsp q:%s function was called.\n", buf);
...
void handle_rsp_X(const char *buf, const size_t buf_len) {
    ...
    for (uint64_t pos = 0; pos < buf_len; ++pos)
        if (buf[pos] == ',')
            unit_size = strtoul(&buf[pos + 1], NULL, 16);
        else if (buf[pos] == ':') { // start of val was found
...
void gdb_stub(RISCVMachine *m, int port_num) {
